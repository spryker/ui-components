...
export class NotificationActionHandlerService implements ActionHandler<unknown, NotificationRef[]> {
    handleAction(
        injector: Injector,
        ...
        for (const data of config.notifications) {
            ...
            notificationData.title = contextService.interpolate(String(notificationData.title), context as AnyContext);
