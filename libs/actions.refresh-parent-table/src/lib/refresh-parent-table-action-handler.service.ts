...
export class RefreshParentTableActionHandlerService implements ActionHandler<unknown, void> {
    handleAction(injector: Injector, config: RefreshParentTableActionConfig, context: unknown): Observable<void> {
        const table = injector.get(CoreTableComponent).parentTable;

        if (!table) {
            throw new Error(`RefreshParentTableActionHandlerService: Parent Table is not found!`);
        }
