...
export class ButtonCore extends ButtonCoreInputs implements AfterViewInit, OnChanges {
    ...
    private setClassList(): void {
        ...
        if (this.buttonRef) {
            this.renderer.addClass(this.buttonRef.nativeElement, `${this.buttonClassName}__btn`);
            this.renderer.addClass(this.buttonRef.nativeElement, `${buttonClassName}__btn`);
        }
    ...
    private changeClassName(type: string, previousValue?: string): void {
        if (previousValue) {
            this.renderer.removeClass(this.elemRef.nativeElement, `${this.buttonClassName}--${previousValue}`);
            this.renderer.removeClass(this.elemRef.nativeElement, `${buttonClassName}--${previousValue}`);
        }

        this.renderer.addClass(this.elemRef.nativeElement, `${this.buttonClassName}--${type}`);
        this.renderer.addClass(this.elemRef.nativeElement, `${buttonClassName}--${type}`);
    }
}
