...
describe('StaticCacheStrategy', () => {
    ...
    beforeEach(() => {
        ...
        cacheStorageFactoryService.create.mockReturnValue(cacheStoragePersistenceAdapter);
    ...
    it('getCached method should cache entry exist in the storage and the date is not expired and return in from cache', () => {
        ...
        const getCachedObservable$ = triggerGetCached$.pipe(
            switchMap((value) => service.getCached(mockId, mockConfig, mockOperations.mockReturnValue(of(value)))),
        );
        ...
        const cachedEntry = cacheStoragePersistenceAdapter.items.get(mockId.serialize());
