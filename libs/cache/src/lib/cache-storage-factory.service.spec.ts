...
class MockPersistenceStrategyService {
    ...
    getAll = jest.fn().mockReturnValue([this.injector.get(MockPersistenceStrategyTypeService)]);
}
...
describe('CacheStorageFactoryService', () => {
    ...
    beforeEach(() => {
        ...
        mockPersistenceStrategyService = TestBed.inject(MockPersistenceStrategyService);
    });

    it('method `create` should return instance of `CacheStoragePersistenceAdapter` that uses a `PersistenceStrategy`', () => {
        const mockConfig = { storage: 'test', type: 'test' };
        const returnedClass = service.create(mockConfig);
        const expectedClass = new CacheStoragePersistenceAdapter(new MockPersistenceStrategyTypeService());

        expect(mockPersistenceStrategyService.select).toHaveBeenCalledWith(mockConfig.storage);
        expect(returnedClass instanceof CacheStoragePersistenceAdapter).toBeTruthy();
        expect(JSON.stringify(returnedClass)).toBe(JSON.stringify(expectedClass));
    });

    it('method `create` should sore return `CacheStoragePersistenceAdapter` instance and reuse it for the same type', () => {
    ...
    it('method `getAll` should return array of instances of `CacheStoragePersistenceAdapter` that uses a `PersistenceStrategy`', () => {
        const returnedClasses = service.createAll();
        const expectedClass = new CacheStoragePersistenceAdapter(new MockPersistenceStrategyTypeService());

        expect(mockPersistenceStrategyService.getAll).toHaveBeenCalled();
        expect(returnedClasses[0] instanceof CacheStoragePersistenceAdapter).toBeTruthy();
        expect(JSON.stringify(returnedClasses)).toBe(JSON.stringify([expectedClass]));
    });
});
