...
describe('CacheStoragePersistenceAdapter', () => {
    ...
    beforeEach(() => {
        ...
        instance = new CacheStoragePersistenceAdapter(mockPersistenceStrategyTypeService);
    });
    ...
    it('get method must return cache entry with namespace from PersistenceStrategies using PersistenceStrategy.retrieve()', () => {
        ...
        expect(mockPersistenceStrategyTypeService.retrieve).toHaveBeenCalledWith(name);
    ...
    it('set method must save cache entry with namespace in PersistenceStrategies and add cache entry with namespace to the Manifest', () => {
        ...
        const name = `${CacheStoragePersistenceAdapter.DefaultNamespace}.${cacheId.serialize()}`;
        ...
        expect(mockPersistenceStrategyTypeService.save).toHaveBeenCalledWith(name, cacheEntry);
    });
    ...
    it('remove method must remove cache entry with namespace in PersistenceStrategies using PersistenceStrategy.remove() and remove cache entry with namespace from the Manifest', () => {
        ...
        expect(mockPersistenceStrategyTypeService.remove).toHaveBeenCalledWith(name);
    });
