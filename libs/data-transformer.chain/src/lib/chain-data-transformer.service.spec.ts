...
import { DataTransformerService, DataTransformerConfig } from '@spryker/data-transformer';
import { of } from 'rxjs';
...
class MockDataTransformerService {
    transform = jest.fn().mockImplementation((value: string, config: DataTransformerConfig) => of(config.value));
}
...
describe('ChainDataTransformerService', () => {
    ...
    it('transform method should map config and use transformers sequently for data transformation and return result', () => {
        ...
        const transformObservable$ = service.transform(mockData, mockConfig, mockInjector);
