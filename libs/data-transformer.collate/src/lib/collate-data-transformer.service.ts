...
import { Observable, of, map, switchMap, delay } from 'rxjs';
    ...
    implements DataTransformer<CollateDataTransformerData, CollateDataTransformerDataT>
{
    ...
    transform(
        ...
        return this.collateDataConfigurator.resolve(config.configurator, injector).pipe(
                ...
                this.filterData(config, configurator, data).pipe(
                    map((filteredData) => {
                        ...
                        if (configurator.pageSize && configurator.page) {
                            copiedData = this.paginateData(configurator.page, configurator.pageSize, copiedData);
                        }

                        if (sortingConfig?.sortBy && sortingConfig?.sortDirection) {
                            copiedData.sort((a, b) => {
                                const comparison = this.sortData(a, b, sortingConfig.sortBy as string);
    ...
    private filterData(
        ...
        const filters = configuratorConfig.filter && collateConfig.filter ? Object.entries(collateConfig.filter) : [];
    ...
    private paginateData(page: number, pageSize: number, data: CollateDataTransformerData): CollateDataTransformerData {
