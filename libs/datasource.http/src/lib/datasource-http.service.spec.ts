...
import { DefaultContextSerializationModule, DiEncodingCodecToken } from '@spryker/utils';
...
describe('DatasourceHttpService', () => {
    ...
    it('resolve method should sent request with http params by `new HttpParams` if `config.dataIn === DatasourceHttpConfigDataIn.Params`', () => {
        ...
        const htmlResponse = httpTestingController.expectOne(`${mockConfig.url}?${mockParams.toString()}`);
