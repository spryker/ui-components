...
export class DatePickerComponent implements OnChanges, AfterViewChecked {
    ...
    private convertEnableTimeFuncToFunc(enableTime: EnableTimeFunction): void {
        this.disabledTime = (date: Date): NzDisabledTimeConfig => {
            ...
            const nzDisabledHours = () => {
                ...
                return DatePickerComponent.HoursRange.filter((hour) => !enabledHours.includes(hour));
            };
            const nzDisabledMinutes = (hour?: number) => {
                const enabledMinutes = enableTimeConfig.minutes(hour);

                return DatePickerComponent.MinutesRange.filter((minute) => !enabledMinutes.includes(minute));
            };
    ...
    private convertEnableDateObjToFunc(enableDateObj: EnableDate): void {
        ...
        const fromDate = convertedEnableDate.from && new Date(convertedEnableDate.from.getTime());
        const toDate = convertedEnableDate.to && new Date(convertedEnableDate.to.getTime());
        ...
        this.disabledDate = (date: Date): boolean => {
            ...
            if (convertedEnableDate.onlyWorkDays && !this.dateWorkDaysToken.includes(date.getDay())) {
                return true;
            }
    ...
    private convertEnableTimeObjToFunc(enableTime: EnableTime): void {
        ...
        if (!convertedEnableTime.onlyWorkHours) {
            const filteredHoursFromTo = this.getTimeRange(DatePickerComponent.HoursRange, fromHours, toHours);

            filteredHoursFromTo.forEach((hour) => enabledHours.add(hour));
        } else {
            const filteredOnlyWorkHoursFromTo = this.filterHoursRange(workHoursRanges, DatePickerComponent.HoursRange);

            filteredOnlyWorkHoursFromTo.forEach((hour) => enabledHours.add(hour));
        }

        const disabledHours = DatePickerComponent.HoursRange.filter((hour) => !enabledHours.has(hour));

        this.disabledTime = (): NzDisabledTimeConfig => {
            const nzDisabledHours = () => disabledHours;
            const nzDisabledMinutes = (hour?: number) =>
                this.filterMinutesRange(workHoursRanges, DatePickerComponent.MinutesRange, hour);
            const nzDisabledSeconds = () => [];
    ...
    private filterMinutesRange(workHoursRanges: number[][][], minutesFromTo: number[], hour?: number): number[] {
        if (hour === undefined) {
            return [];
        }
    ...
    private filterHoursRange(workHoursRanges: number[][][], hoursFromTo: number[]): number[] {
