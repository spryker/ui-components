...
export class InterceptionService implements InterceptorDispatcher, Interceptor {
    ...
    private getHandlersForEvent(event: InterceptionEventType<unknown>): Observable<InterceptionHandler<any>[]> {
    ...
    dispatch(event: InterceptionEventType<unknown>, data?: unknown): Observable<unknown> {
        return this.getHandlersForEvent(event).pipe(
            switchMap((h) =>
                h.reduce((prev$, handler) => prev$.pipe(switchMap((handlerData) => handler(handlerData))), of(data)),
            ),
        );
    }
    ...
    intercept<D>(event: InterceptionEventType<D>, handler: InterceptionHandler<D>): Observable<void> {
        return new Observable((subscriber) => {
            ...
            this.handlersMap$.next(this.handlersMap$.getValue().set(event, [...handlers, handler]));

            return () => {
                const updatedHandlers = this.handlersMap$.getValue().get(event) || [];
                const filteredHandlers = updatedHandlers.filter((h) => h !== handler);

                this.handlersMap$.next(this.handlersMap$.getValue().set(event, filteredHandlers));
            };
        });
    }
}
...
export abstract class InterceptorDispatcherService implements InterceptorDispatcher {
    abstract dispatch<D>(event: InterceptionEventType<D>, data: D): Observable<D>;
    abstract dispatch<D extends never>(event: InterceptionEventType<D>): Observable<void>;

    abstract dispatchToAll<D>(event: InterceptionEventType<D>, data: D): Observable<D>;
    abstract dispatchToAll<D extends never>(event: InterceptionEventType<D>): Observable<void>;
}
...
export abstract class InterceptorService implements Interceptor {
    abstract intercept<D>(event: InterceptionEventType<D>, handler: InterceptionHandler<D>): Observable<void>;
}
