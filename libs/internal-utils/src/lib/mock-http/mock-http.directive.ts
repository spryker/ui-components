...
export class MockHttpDirective implements DoCheck, OnDestroy {
  ...
  private getIsErrorFrom({ response }: MatchedHttpRequest): boolean {
    const errorRate =
      response.errorRate ??
      this.mockHttpOptions?.errorRate ??
      MockHttpDirective.options.errorRate;

    return errorRate > 0 && Math.random() > errorRate;
  }

  private getDataFrom<D>({
    request,
    response,
  ...
  private getDelayFrom({ response }: MatchedHttpRequest): number {
    const delayValue =
      response.delay ??
      this.mockHttpOptions?.delay ??
      MockHttpDirective.options.delay;

    const maxDelay =
      response.maxDelay ??
      this.mockHttpOptions?.maxDelay ??
      MockHttpDirective.options.maxDelay;
