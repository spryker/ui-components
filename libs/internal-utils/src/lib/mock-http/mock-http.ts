...
export type MockHttpResolver<T = unknown> = (request: TestRequest, injector: Injector) => MockHttpValue<T>;
...
export interface MockHttpResponse<D = unknown, E = unknown> extends MockHttpOptions {
    url: string | RegExp;
    method?: string;
...
export function isMatchedRequest(req: Partial<MatchedHttpRequest>): req is MatchedHttpRequest {
    return !!req.response;
}
