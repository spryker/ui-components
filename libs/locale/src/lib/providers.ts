...
import { AnyFunction, FunctionWithArgs, FunctionWithoutArgs } from '@spryker/utils';
...
export function provideLocaleRecordsFactory<F extends FunctionWithArgs<LocaleRecord[]>>(
    factory: F,
    deps: Parameters<F>,
): Provider;
export function provideLocaleRecordsFactory<F extends FunctionWithoutArgs<LocaleRecord[]>>(factory: F): Provider;
export function provideLocaleRecordsFactory(factory: AnyFunction<LocaleRecord[]>, deps?: any[]): Provider {
    return {
        provide: LocaleRecordsToken,
        useFactory: factory,
        deps,
...
export function provideLocaleLoaderRegistrars(registrars: Partial<LocaleLoaderRegistrarMap>): Provider {
    return {
