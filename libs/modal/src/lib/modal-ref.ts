...
export type ModalRenderingFn<T> = (modalRef: ModalRef<T, any>) => ModalRenderingRef<T, any>;
...
export class ModalRefImpl<T> extends ModalRef<T, any> {
    ...
    updateHtml(html: string | HTMLElement | Node): void {
        ...
        if (this.renderingRef && this.renderingRef.updateHtml) {
            this.renderingRef.updateHtml(html);
        } else {
