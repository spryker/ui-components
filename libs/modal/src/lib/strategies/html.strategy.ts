...
import { ModalRenderingRef, ModalStrategy, ModalRef, InferModalData } from '../types';
...
export interface HtmlModalStrategyOptionsElement<T> extends HtmlModalStrategyOptionsBase<T> {
    html: HTMLElement | Node | HtmlFactory<HTMLElement | Node>;
    cloneNode?: true;
}

export interface HtmlModalStrategyOptionsString<T> extends HtmlModalStrategyOptionsBase<T> {
...
export class HtmlModalStrategy<T> implements ModalStrategy<T, HtmlModalExtras> {
    ...
    render(vcr: ViewContainerRef, modalRef: ModalRef<T>): HtmlModalRenderingRef<T> {
        ...
        const render: HtmlModalRenderingFunction = (data) => {
            ...
            nodes.reduceRight((prevNode, node) => parentElement.insertBefore(node, prevNode), renderElement);
    ...
    private selectRenderFn(): HtmlNodesFactory<T> {
        return ('element' in this.options ? this.renderElement : this.renderString) as HtmlNodesFactory<T>;
    }

    private resolveHtml<H extends HtmlType>(html: H | HtmlFactory<H>, modalData: any): H {
    ...
    private renderElement(options: HtmlModalStrategyOptionsElement<T>, html: HTMLElement): Node[] {
    ...
    private renderString(options: HtmlModalStrategyOptionsString<T>, html: string): Node[] {
        const element = document.createElement('div');
