...
export type InferModalResult<T> = T extends Modal<any, infer Result> ? Result : never;
...
export interface ModalStrategy<T, TExtra = never> {
    render(vcr: ViewContainerRef, modalRef: ModalRef<T, TExtra>): ModalRenderingRef<T, TExtra>;
}
