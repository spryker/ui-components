...
export class NotificationComponent extends NotificationInputs implements OnChanges, AfterViewInit, OnDestroy {
    ...
    private floatingClosed$ = this.floatingRef$.pipe(
        switchMap((floatingRef) => floatingRef.afterClose().pipe(take(1), takeUntil(this.closeFloating$))),
        tap(() => this.close()),
    );
    ...
    ngAfterViewInit(): void {
        this.floatingSink$.pipe(takeUntil(this.destroyed$)).subscribe(() => this.cdr.markForCheck());
