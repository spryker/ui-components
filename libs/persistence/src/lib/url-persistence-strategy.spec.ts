...
describe('UrlPersistenceStrategy', () => {
    ...
    const queryStringGeneration = (urlParams: URLSearchParams) =>
        `${windowToken.location.pathname}?${urlParams.toString() + `#${windowToken.location.hash}`}`;
    ...
    it('save method must store `value` under `key` in the URL Query', () => {
        ...
        expect(windowToken.history.pushState).toHaveBeenCalledWith({}, mockKey, queryStringGeneration(urlParams));
    });
