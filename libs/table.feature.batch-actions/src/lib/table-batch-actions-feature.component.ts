...
export class TableBatchActionsFeatureComponent extends TableFeatureComponent<TableBatchActionsConfig> {
    ...
    itemActions$: Observable<TableItemActions> = this.itemSelected$.pipe(
        ...
        map(([selectedRows, config]) => {
            ...
            const actions = config.availableActionsPath
                ? this.getAvailableActions(selectedRows, config.actions, config.availableActionsPath)
                : config.actions;
    ...
    shouldShowActions$ = this.itemActions$.pipe(
        map((itemActions) => Boolean(itemActions.actions.length && itemActions.selectedRows.length)),
        shareReplay({ bufferSize: 1, refCount: true }),
    );
    noActionsMessage$ = this.itemActions$.pipe(
        withLatestFrom(this.config$),
        map(([itemActions, config]) => {
            const shouldShowNotification = Boolean(!itemActions.actions.length && itemActions.selectedRows.length);

            return shouldShowNotification ? config.noActionsMessage : undefined;
        }),
        shareReplay({ bufferSize: 1, refCount: true }),
    );
    styles$ = combineLatest([this.shouldShowActions$, this.noActionsMessage$]).pipe(
    ...
    private getAvailableActions(
        ...
        const availableActionIds = selectedRows.reduce((ids: string[][], row) => {
            const availableActions = this.contextService.interpolateExpression(availableActionsPath, row.data as any);

            return !Array.isArray(availableActions) ? ids : [...ids, availableActions];
        }, []);
        ...
        const intersection: Record<string, boolean> = multipleIntersectionOfString(availableActionIds).reduce(
    ...
    buttonClickHandler(action: TableBatchAction, batchAction: TableItemActions) {
        ...
        const rowIds = batchAction.selectedRows.reduce(
            (ids: string[], row) => [
                ...ids,
                this.contextService.interpolateExpression(batchAction.rowIdPath, row.data as any) as string,
            ],
            [],
        );
        ...
        this.tableActionService.trigger(batchEvent, context);
    }
}
