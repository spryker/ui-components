...
describe('TableEditableFeatureComponent', () => {
    ...
    describe('Create Section', () => {
        ...
        it('should sync `config.create.initialData.data` to `ngModel` of the `input[type=hidden]` element`', () => {
            ...
            expect(inputElem.properties.ngModel).toBe(JSON.stringify(mockConfig.create.initialData.data));
        });
        ...
        it('should render `spy-button` with `config.create.addButton.title` text', () => {
            ...
            expect(createButtonElem.nativeElement.textContent).toContain(mockConfig.create.addButton.title);
        });
        ...
        it('should render `spy-icon` inside `spy-button` with `config.create.addButton.icon` name', () => {
            ...
            expect(createIconElem.properties.name).toBe(mockConfig.create.addButton.icon);
        });
        ...
        it('should render `spy-button` with predefined options', () => {
            ...
            expect(createButtonElem.classes[`${buttonClassName}--${mockConfig.create.addButton.variant}`]).toBeTruthy();
            expect(createButtonElem.classes[`${buttonClassName}--${mockConfig.create.addButton.shape}`]).toBeTruthy();
            expect(createButtonElem.classes[`${buttonClassName}--${mockConfig.create.addButton.size}`]).toBeTruthy();
        });

        it('should render additional `th` and `td` if `createConfig.cancelButton` exist', () => {
            const thElem = fixture.debugElement.query(By.css('.spy-table-editable-feature__th'));
            const tdElem = fixture.debugElement.query(By.css('.spy-table-editable-feature__td'));
        ...
        it('should render `ant-table-row spy-table-editable-feature__row` with `spy-table-editable-feature__cell` if `config.create.initialData.data` exist', () => {
            const cellElem = fixture.debugElement.query(
                By.css('.spy-table-editable-feature__row .spy-table-editable-feature__cell'),
            );
        ...
        it('should render `spy-icon` inside `spy-button` with `config.create.cancelButton.icon` name', () => {
            ...
            expect(cancelIconElem.properties.name).toBe(mockConfig.create.cancelButton.icon);
        });
        ...
        it('should render additional `tr` with `spy-table-editable-feature__error` if `config.create.initialData.errors[index].rowError` exist', () => {
            const errorTrElem = fixture.debugElement.query(By.css('tr .spy-table-editable-feature__error'));

            expect(errorTrElem).toBeTruthy();
            expect(errorTrElem.nativeElement.textContent).toContain(mockConfig.create.initialData.errors[0].rowError);
        });

        it('should render default `td` value if `disableForCols` has id of this column', () => {
            const rowElem = fixture.debugElement.query(By.css('.spy-table-editable-feature__row'));
            const cellElems = rowElem.queryAll(By.css('.spy-table-editable-feature__cell spy-table-column-renderer'));
        ...
        it('should render default `td` values if `disableRowKey` has id of any column in row', () => {
            const rowElem = fixture.debugElement.queryAll(By.css('.spy-table-editable-feature__row'))[1];
            const cellElems = rowElem.queryAll(By.css('.spy-table-editable-feature__cell spy-table-column-renderer'));
