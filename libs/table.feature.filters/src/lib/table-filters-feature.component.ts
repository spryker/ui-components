...
export class TableFiltersFeatureComponent extends TableFeatureComponent<TableFiltersConfig> {
    ...
    filterComponentMap: Record<string, Type<TableFilterComponent<TableFilterBase>>> = this.tableFilterToken.reduce(
        ...
        map(([filterValues, updatedValue]) => {
            ...
            const parsedFilter = Object.fromEntries(
                Object.entries(filter).map(([key, value]) => {
                    const parsedValue = value && !isNaN(value as any) ? JSON.stringify(value) : value;
