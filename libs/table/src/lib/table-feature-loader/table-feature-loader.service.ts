...
export class TableFeatureLoaderService implements OnDestroy {
  ...
  constructor(
    ...
    @Optional()
    private featuresRegistries: InjectionTokenType<
      typeof TableFeaturesRegistryToken
    > | null,
  ...
  loadFactoriesFor(
    ...
    const featureModules = featureNames.reduce(
      ...
      Object.create(null) as Record<
        string,
        Observable<ComponentFactory<TableFeatureComponent>>
      >,
    );
  ...
  private mapFeatures<T, R>(
    features: Record<string, T>,
    mapFn: (feature: T) => R,
  ): Record<string, R> {
    return Object.fromEntries(
      Object.entries(features).map(([name, feature]) => [name, mapFn(feature)]),
    );
  }
}
