...
export class TableFeaturesRendererService implements OnDestroy {
    private featureLocationStore = new TokenStore<[TableFeatureComponent[]], Observable<string[][][]>>((features) =>
        combineLatest(
            features.map((feature) =>
                feature.tplDirectives$.pipe(
                    switchMap((tplDirectives) =>
                        combineLatest(tplDirectives.map((tplDirective) => tplDirective.locations$)),
                    ),
                    debounceTime(0),
                ),
            ),
    ...
    trackFeatureRecords(
        ...
        return this.featureRecordsStore.resolve(features, location, this.featureLocationStore.resolve(features));
    }
    ...
    chainFeatureContexts<TContext, TArgs extends any[]>(
        ...
        const featureContexts$ = featuresInLocation$.pipe(
            switchMap((features) => {
                ...
                return combineLatest(features.map((feature) => feature.featureContext$ ?? of(undefined)));
            }),
            startWith([]),
            debounceTime(0),
        );
...
class TokenStore<T extends any[], D> {
    ...
    resolve(...tokens: T): D {
        const args = this.args.find(
            (arg) => tokens.length === arg[0].length && tokens.every((token, i) => token === arg[0][i]),
        );
