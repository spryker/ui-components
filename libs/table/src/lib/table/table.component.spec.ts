...
describe('TableComponent', () => {
    ...
    describe('spy-table-features-renderer', () => {
        ...
        it('must render features in the appropriate blocks', fakeAsync(async () => {
            ...
            const topFeaturesElem = host.queryCss('.ant-table-features--top .top-feature');
            const beforeTableFeaturesElem = host.queryCss('.ant-table-features--before-table .before-table-feature');
            const afterTableFeaturesElem = host.queryCss('.ant-table-features--after-table .after-table-feature');
            const bottomFeaturesElem = host.queryCss('.ant-table-features--bottom .bottom-feature');
            const hiddenFeaturesElem = host.queryCss('.ant-table-features--hidden .hidden-feature');
            const headerExtFeaturesElem = host.queryCss('thead th:last-child .header-ext-header-feature');
            const beforeColsHeaderFeaturesElem = host.queryCss('thead th:first-child .before-cols-header-feature');
            const headerFeaturesElems = host.fixture.debugElement.queryAll(By.css('thead th .header-feature'));
            const afterColsHeaderFeaturesElem = host.queryCss('thead th:nth-child(5) .after-cols-header-feature');
            const beforeRowsFeaturesElem = host.queryCss('tbody tr:nth-child(1).before-rows-feature');
            const beforeColsFeaturesElem = host.queryCss('tbody tr:nth-child(2) td:first-child .before-cols-feature');
            const cellFeaturesElems = host.fixture.debugElement.queryAll(
                By.css('tbody tr:nth-child(2) td .cell-feature'),
            );
            const afterColsFeaturesElem = host.queryCss('tbody tr:nth-child(2) td:nth-child(5) .after-cols-feature');
            const afterRowsFeaturesElem = host.queryCss('tbody tr:nth-child(3).after-rows-feature');
    ...
    describe('@Input(config)', () => {
        ...
        describe('columnsUrl dataSourceUrl columns', () => {
            ...
            it('prop data$ must be mapped into <tbody> and render <spy-table-column-renderer> at each <td>', fakeAsync(async () => {
                ...
                const columnElement = host.queryCss('tr:first-child td:first-child spy-table-column-renderer');
            ...
            it('prop columns$ must be mapped into <thead> and create with <tr> and each <th> of the <table>', fakeAsync(async () => {
                ...
                expect(thElements[0].nativeElement.textContent).toMatch(mockCols[0].title);
            }));
        });
    });
    ...
    function verifyDataRequest() {
        const dataReq = httpTestingController.expectOne((req) => req.url.includes(mockDataUrl));
