...
export class CoreTableComponent implements TableComponent, OnInit, OnChanges, AfterContentInit, OnDestroy {
    ...
    @ContentChildren(ColTplDirective) set slotTemplates(val: QueryList<ColTplDirective>) {
    ...
    columns$ = this.columnsConfig$.pipe(
        switchMap((colsOrUrl) =>
            colsOrUrl
                ? this.columnsResolverService.resolve(colsOrUrl).pipe(catchError(this.handleStreamError()))
                : of([]),
        ),
        shareReplaySafe(),
    );
    ...
    featureFactories$ = this.config$.pipe(switchMap((config) => this.featureLoaderService.loadFactoriesFor(config)));
    ...
    featureCellContext$ = this.tableFeaturesRendererService.chainFeatureContexts(
        ...
        () => this.cellTpl,
        (config: TableColumn, row: TableDataRow, i: number, j: number): TableColumnContext => ({
            config,
            row,
            i,
            j,
    ...
    featuresInRows$ = this.features$.pipe(
        switchMap((features) =>
            combineLatest([
                this.tableFeaturesRendererService.trackFeatureRecords(features, TableFeatureLocation.beforeRows),
                this.tableFeaturesRendererService.trackFeatureRecords(features, TableFeatureLocation.afterRows),
            ]),
        ),
    ...
    isEmpty$ = combineLatest([
        ...
        this.dataConfiguratorService.config$.pipe(map((config) => Boolean(Object.keys(config).length))),
    ...
    isNotFiltered$ = this.dataConfiguratorService.config$.pipe(
        ...
        map(([prevConfig, newConfig]) => {
            const isFiltered = (config: Record<string, unknown>) =>
                config.filter ? Boolean(Object.keys(config.filter as Record<string, unknown>).length) : false;
            const isSearched = (config: Record<string, unknown>) =>
                config.search ? String(config.search).length : false;
            const isChanged =
                isFiltered(prevConfig) || isSearched(prevConfig) || isFiltered(newConfig) || isSearched(newConfig);
    ...
    private featuresDiffer = this.iterableDiffers.find([]).create<TableFeatureComponent>();
    ...
    findFeatureByType<T extends TableFeatureComponent>(type: Type<T>): Observable<T> {
        return this.features$.pipe(
    ...
    rowClickHandler(row: TableDataRow, event: Event): void {
        this.tableEventBus.emit<TableRowClickEvent>('table', { row, event }, 'row-click');
    }
    ...
    private sortingValueTransformation(value: string | null): SortingCriteria['sortDirection'] {
        if (value === 'descend') {
            return 'desc';
        }
    ...
    private initFeature(feature: TableFeatureComponent): void {
        ...
        feature.setTableEventBus(new TableFeatureEventBus(feature.name, this.tableEventBus));
