...
export abstract class UnsavedChangesGuardBase implements OnDestroy, UnsavedChangesGuard {
    ...
    protected monitorStatuses$ = this.monitors$.pipe(
        switchMap((monitors) =>
            monitors.size
                ? combineLatest([...monitors].map((monitor) => monitor.getStatus().pipe(distinctUntilChanged())))
                : of([]),
        ),
    );

    protected hasDirtyStatus$ = this.monitorStatuses$.pipe(
        map((statuses) => statuses.some((status) => status === UnsavedChangesMonitorStatus.Dirty)),
