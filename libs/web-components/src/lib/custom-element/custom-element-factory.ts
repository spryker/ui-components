...
import { Observable, Subject, map, take, takeUntil, shareReplay } from 'rxjs';
...
export function createCustomElementForStatic<T>(
    ...
    const defaultStrategyFactory = new DefaultElementBoundaryNgElementStrategyFactory(componentType, injector);

    const strategyFactory = new CrossBoundaryNgElementStrategyFactory(defaultStrategyFactory, {
    ...
    class WebComponent extends (NgElement as any) implements NgWebComponent<any> {
        ...
        private whenInit$ = elementBoundaryService.whenBoundaryExist(this as any, timeoutMs).pipe(
            take(1),
            map(() => void 0),
        ...
        private __init() {
            elemMethods.forEach((method) => exposeMethod(this, this.ngComponent, method, Object.getPrototypeOf(this)));
        }
    }
...
export function createCustomElementForLazy<T>(
    ...
    class LazyWebComponent extends (HTMLElement as any) implements NgWebComponent<any> {
        private destroyed$ = new Subject<void>();
            ...
            this.whenInit()
                ...
                .subscribe(() => {
                    ...
                    elemMethods.forEach((method) =>
                        exposeMethod(this, this.ngComponent, method, Object.getPrototypeOf(this)),
                    );
                });
        }
    }
...
function getComponentMethods(componentDeclaration: WebComponentDeclaration): string[] {
